 ...
 */
package com.min.uitls.common;

/**
 * 
 * @author Jason.Wang
 * @version $Id: MD5.java, v 0.1 2015年6月30日 下午5:11:54 Jason.Wang Exp $
 */
public final class MD5 {
    public static void main(String[] args) {
        MD5 md5 = new MD5();
        String s = "admin";
        System.out.println(md5.toDigest(s));
    ...
    public String toDigset16(String string) {
        return toDigest(string).substring(8, 24);
    }

    public String toDigset32(String string) {
        return toDigest(string);
    }
     ...
     * @return
     */
    private String toDigest(String string) {
    ...
    private byte[] toDigest(byte[] string) {
    ...
    void Decode(long[] output, byte[] input, int iidx, int len) {
        int i, j;
        for (i = 0, j = 0; j < len; i++, j += 4)
            output[i] = ((long) byte2int(input[iidx + j])) | (((long) byte2int(input[iidx + j + 1])) << 8) | (((long) byte2int(input[iidx + j + 2])) << 16) | (((long) byte2int(input[iidx + j + 3])) << 24);
    }
